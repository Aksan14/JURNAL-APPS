...
// 1. Ambil Data Guru Saat Ini
$stmt = $pdo->prepare("SELECT * FROM tbl_guru WHERE user_id = ?");
...
if (isset($_POST['update_profil'])) {
    ...
    $nip  = trim($_POST['nip']);
    $foto_name = $guru['foto'] ?? null; // Nama foto lama sebagai default
    ...
    if (!empty($_FILES['foto']['name'])) {
        ...
        if (!in_array(strtolower($file_type), $allowed_type)) {
            ...
            $foto_name = "guru_" . $user_id . "_" . time() . "." . $file_type;
            ...
            if (!empty($guru['foto']) && $guru['foto'] != 'default.png' && file_exists("../assets/img/profile/" . $guru['foto'])) {
                unlink("../assets/img/profile/" . $guru['foto']);
            }
    ...
    // Update Database
    if (empty($message)) {
        $update = $pdo->prepare("UPDATE tbl_guru SET nama_guru = ?, nip = ?, foto = ? WHERE user_id = ?");
        $update->execute([$nama, $nip, $foto_name, $user_id]);
...
<div class="container-fluid">
    ...
    $has_foto = !empty($guru['foto']) && file_exists($foto_path);
    ?>
        ...
        <div class="col-md-4">
            <div class="card shadow mb-4 text-center">
                <div class="card-body">
                    ...
                    <?php endif; ?>
                    <h5 class="font-weight-bold"><?php echo htmlspecialchars($guru['nama_guru'] ?? ''); ?></h5>
